...
use crate::{
    cached_package::{cache_package_to_disk_entry, create_cached_package, find_cached_package, CachedPackageIdentifier}, disk_cache::{path_of_cached_package, write_cached_file, DiskCacheEntry}, error::ApplicationError, fetch::fetch_package, response::AppResponse, state::AppState, statistics::{fetch_statistics, StatisticsTemplate}, upstream::find_upstream_by_name, upstream_mirror::{get_mirrors_for_upstream_id, should_cache_file}
};
...
pub async fn caching_package_endpoint(
    ...
    let Some(upstream) = maybe_upstream else {
        ...
        return Err(ApplicationError::General(anyhow::anyhow!(format!(
            "Upstream with name {} does not exist", upstream_name.clone()
        ))));
    };
